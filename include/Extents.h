...
{
    bool minsOk = glm::all(glm::lessThanEqual(mins, test.maxs));
    bool maxsOk = glm::all(glm::greaterThanEqual(maxs, test.mins));
    return minsOk && maxsOk;
}


inline bool Extents::contains(const Extents& test) const
{
    bool minsOk = glm::all(glm::lessThan(mins, test.mins));
    bool maxsOk = glm::all(glm::greaterThan(maxs, test.maxs));
    return minsOk && maxsOk;
}
