...
void BackgroundRenderer::drawGroundReceivers(SceneRenderer& renderer)
{
    ...
    // Reuse static buffers to avoid dynamic heap allocations per frame
    static std::vector<glm::vec4> allColors;
    static std::vector<glm::vec3> allVerts;
    allColors.clear();
    allVerts.clear();
    ...
    for (int k = 0; k < count; k++)
    {
        ...
        // Compute light intensity directly underneath the source
        float d = posZ;
        float I = B / (atten[0] + d * (atten[1] + d * atten[2]));

        // Check maximum RGB channel brightness
        ...
        // Center Fan: Rendered using a local matrix transform
        glPushMatrix();
        ...
        // Concentric Rings: Batched globally in world space
        // Add degenerate vertices to bridge from the previous light source
        if (!allVerts.empty())
        {
            // Duplicate the last vertex of the previous light ring
            allVerts.push_back(allVerts.back());
            allColors.push_back(allColors.back());

            // Duplicate the first vertex of the new light ring
            color.a = outerAlpha; // innerAlpha for ring 1
            glm::vec3 firstVert = glm::vec3(pos[0] + angle[0].x * receiverRingSize,
                                            pos[1] + angle[0].y * receiverRingSize,
                                            0.0f);
            allVerts.push_back(firstVert);
        ...
        for (int i = 1; i < receiverRings; i++)
        {
            ...
            for (int j = 0; j <= receiverSlices; j++)
            {
                // Apply world space position directly in the buffer
                color.a = innerAlpha;
                allColors.push_back(color);
                allVerts.push_back(glm::vec3(pos[0] + angle[j].x * innerSize,
                                             pos[1] + angle[j].y * innerSize,
                                             0.0f));

                color.a = outerAlpha;
                allColors.push_back(color);
                allVerts.push_back(glm::vec3(pos[0] + angle[j].x * outerSize,
                                             pos[1] + angle[j].y * outerSize,
                                             0.0f));
            }
        }
    ...
    // Issue a single batched draw call for all light receiver rings
    if (!allVerts.empty())
    {
        Vertex_Chunk allRings(Vertex_Chunk::VC, allVerts.size());
        allRings.colorData(allColors);
        allRings.vertexData(allVerts);
        allRings.draw(GL_TRIANGLE_STRIP);
    }
