...
{
    glVertexPointer(3, GL_FLOAT, sizeof(glm::vec3), glm::value_ptr(vertices[0]));
}

void glNormalPointer(const glm::vec3 normals[])
{
    glNormalPointer(GL_FLOAT, sizeof(glm::vec3), glm::value_ptr(normals[0]));
}

void glTexCoordPointer(const glm::vec2 texCoords[])
{
    glTexCoordPointer(2, GL_FLOAT, sizeof(glm::vec2), glm::value_ptr(texCoords[0]));
}
